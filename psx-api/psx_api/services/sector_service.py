...
class SectorService:
    ...
    async def compare(self, sector: str, symbols: list[str]) -> dict[str, Any]:
        ...
        sector_rows = (
            (
                await self._db.execute(
                    select(Security).where(Security.sector == sector, Security.is_active.is_(True))
                )
            )
            .scalars()
            .all()
    ...
    async def heatmap(self, sector: str | None = None) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def _compute_ranks(target: Security, universe: dict[str, Security]) -> dict[str, Any]:
