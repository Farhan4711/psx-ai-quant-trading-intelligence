...
class TestEngine:
    def test_buy_and_hold_captures_full_run(self) -> None:
        ...
        assert bt.final_equity_pkr == pytest.approx(
            100_000 * (bars[-1].close / bars[1].open), rel=0.01
        )
    ...
    def test_commission_reduces_returns(self) -> None:
        ...
        bt_no_commish = run(
            bars,
            strategy=long_then_flat,
            initial_capital_pkr=10_000,
            commission_per_side_pct=0.0,
            warmup_bars=0,
        )
        bt_commish = run(
...
class TestNarration:
    ...
    def test_lessons_handles_no_trades(self) -> None:
        ...
        bars = _bars([100.0] * 50)
        bt = run(bars, strategy=lambda _b, _i: 0, initial_capital_pkr=10_000, warmup_bars=0)
        bench = buy_and_hold_return_pct(bt)
        m = compute_metrics(bt, benchmark_return_pct=bench)
        ls = lessons(m, strategy_label="Test")
        assert any("never triggered" in msg for msg in ls)
