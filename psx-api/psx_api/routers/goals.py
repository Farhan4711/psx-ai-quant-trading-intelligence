...
def _with_analysis(goal_dict: dict, analysis: GoalAnalysisResponse) -> GoalWithAnalysis:
...
async def list_goals(db: DbDep, current_user: CurrentUser) -> list[GoalWithAnalysis]:
    ...
    for g in goals:
        ...
        out.append(
            _with_analysis(GoalResponse.model_validate(g).model_dump(), analysis)
        )
    return out
...
async def create_goal(
    body: GoalCreate, db: DbDep, current_user: CurrentUser
) -> GoalWithAnalysis:
    from psx_api.models.users import User
    user: User = current_user  # type: ignore[assignment]
    service = GoalService(db)
...
async def get_goal(
    goal_id: str, db: DbDep, current_user: CurrentUser
) -> GoalWithAnalysis:
    from psx_api.models.users import User
    user: User = current_user  # type: ignore[assignment]
    service = GoalService(db)
